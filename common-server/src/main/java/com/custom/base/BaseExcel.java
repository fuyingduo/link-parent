...
public abstract class BaseExcel<T> {
     ...
     * @return 实体数组
     */
    protected ExcelRt<List<T>> dealWith(final InputStream inputStream, final Object cls) {
        ...
        final XSSFSheet firstSheet = workbook.getSheetAt(FIRST_SHEET);
        final int lastRowNum = firstSheet.getLastRowNum();
        if (MAX_ROW < lastRowNum) {
            final String err = String.format("[BaseExcel] 不能超过:{%s}行, 当前总行数:{%s}", MAX_ROW, lastRowNum);
            BaseExcel.LOG.error(err);
            return ExcelRt.err(err);
        }
        final int cellNum = firstSheet.getRow(FIRST_ROW).getPhysicalNumberOfCells();
        if (MAX_LINE < cellNum) {
            final String err = String.format("[BaseExcel] 不能超过:{%s}列, 当前总列数:{%s}", MAX_ROW, cellNum);
            BaseExcel.LOG.error("[BaseExcel] 不能超过:{}列, 当前总列数:{}", MAX_ROW, cellNum);
            return ExcelRt.err(err);
        }
        final Field[] declaredFields = cls.getClass().getDeclaredFields();
        if (cellNum != declaredFields.length) {
            final String err = String.format("[BaseExcel] 传入模版列数:{%s} 于当前实体列数:{%s} 长度不一致!", cellNum, declaredFields.length);
            BaseExcel.LOG.error(err);
            return ExcelRt.err(err);
        }
        final List<T> objects = Collections.synchronizedList(new ArrayList<>());
        final List<String> fields = Collections.synchronizedList(new ArrayList<>());
        for (final Row row : firstSheet) {
            final Iterator<Cell> cellIterator = row.cellIterator();
            if (FIRST_ROW == row.getRowNum()) {
                final String simpleName = cls.getClass().getSimpleName();
    ...
    private void assignment(final Cell cell, final Field field, final Object testClass) {
        try {
    ...
    private Class<T> getMessageType() {
        Class<T> messageType;
        final TypeToken tt = TypeToken.of(this.getClass());
        final TypeToken btt = tt.getSupertype(BaseExcel.class);
